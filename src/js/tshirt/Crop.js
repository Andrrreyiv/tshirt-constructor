...
/**
 * CSS для самой картинки мокапа. Требование: выбранный кадр обязан ЗАПОЛНИТЬ контейнер
 * ровно: точка crop.x уходит в левый край, а crop.x + crop.w — в правый. Только тогда
 * рамка зоны печати, которую zoneInCrop считает в долях ВИДИМОЙ части, ляжет на то же
 * место изделия, что и до кадрирования.
 *
 * ⚠️ Так было НЕ всегда. Прежняя версия масштабировала от центра кадра и не сдвигала
 * картинку. Центр кадра оставался там же, где был в исходном изображении, вместо
 * центра контейнера, поэтому кадр уезжал. Совпадение получалось ТОЛЬКО у
 * центрированного кадра (crop.x + crop.w/2 = 0.5).
 * Замер на боевом 31.07 при crop {x:0, y:0, w:0.78} и ширине сцены 314 px: кадр
 * ложился в −35…279 вместо 0…314, то есть синяя рамка зоны печати разъезжалась
 * с футболкой на 35 px. Старый тест этого не ловил: он сверял строки со СВОЕЙ же
 * реализацией, а один из двух его случаев был центрированным, где ошибки нет.
 *
 * Сдвиг и масштаб от левого верхнего угла: translate применяется первым (правее в записи),
 * проценты translate считаются от собственной ширины картинки, поэтому
 * x' = (p·W − crop.x·W) · k. При p = crop.x даёт 0, при p = crop.x + crop.w даёт W.
 *
 * Картинка остаётся в потоке и задаёт размер контейнера, поэтому кадрирование
 * не ломает раскладку — лишнее обрезается overflow:hidden у контейнера.
...
export function mockupTransform(crop) {
  ...
  return {
    transformOrigin: '0 0',
    transform: 'scale(' + round(k) + ') translate('
      + round(-crop.x * 100) + '%, ' + round(-crop.y * 100) + '%)',
  };
}
